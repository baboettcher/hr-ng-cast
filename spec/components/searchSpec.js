...
describe('search', function() {
  ...
  it('should have a result function on the scope', function() {
    expect(element.isolateScope().result).to.exist;
    expect(element.isolateScope().result).to.be.a('function');
  });

  it('should not use & function binding', function() {
    expect(element.isolateScope().result).to.equal(resultSpy);
  });
  ...
  it('should invoke the result function with the results of the search', function() {
    element.find('button').click();
    expect(element.isolateScope().result.callCount).to.equal(1);
  });


  // ADVANCED CONTENT TEST
  it('should have access to a search service within the scope', function() {
    expect(element.isolateScope().service).to.exist;
    expect(element.isolateScope().service).to.be.a('object');
  });
